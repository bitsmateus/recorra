...
export default function IntegracoesPage() {
  ...
  async function testar(id: string) {
    ...
    try {
      ...
      setMsg(r.ok ? '✓ Conexão OK' : `Falha na conexão${r.erro ? `: ${r.erro}` : ''}`);
      carregar();
          ...
          {lista.map((i) => (
            <div key={i.id} className="rounded-lg border border-line bg-surface p-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => testar(i.id)} className="rounded border border-line px-3 py-1.5 text-xs hover:bg-canvas">Testar</button>
...
function NovaIntegracaoModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [sistema, setSistema] = useState(ERPS[0].id);
  const [urlBase, setUrlBase] = useState('');
  const [vals, setVals] = useState<Record<string, string>>({});
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState('');
  const erp = ERPS.find((e) => e.id === sistema)!;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-surface p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-ink">Adicionar integração de origem</h2>
        ...
        <label className="mb-3 block text-sm"><span className="mb-1 block text-xs text-muted">Sistema (ERP)</span>
          <select value={sistema} onChange={(e) => { setSistema(e.target.value); setVals({}); }} className={inputCls}>
        ...
        {erp.campos.map((c) => (
          <label key={c.key} className="mb-3 block text-sm"><span className="mb-1 block text-xs text-muted">{c.label}</span>
            <input value={vals[c.key] || ''} onChange={(e) => set(c.key, e.target.value)} placeholder={c.placeholder} className={inputCls} />
          </label>
        ))}
